...
async function createRecord(req, res) {
    try {
        ...
        if (error.errors.name)
            ...
            res.send({ status: 400, result: "Fail",error:error, message: error.errors.maincategory.message })
        else if (error.errors.subcategory)
            res.send({ status: 400, result: "Fail",error:error, message: error.errors.subcategory.message })
            ...
            res.send({ status: 400, result: "Fail",error:error, message: error.errors.baseprice.message })
        else if (error.errors.discount)
            res.send({ status: 400, result: "Fail",error:error, message: error.errors.discount.message })
        else if (error.errors.finalprice)
            res.send({ status: 400, result: "Fail",error:error, message: error.errors.finalprice.message })
        else
            res.send({ status: 500, result: "Fail",error:error, message: "Internal Server Error",error:error })
    }
}
...
async function updateRecord(req, res) {
    try {
        ...
        if (error.keyValue)
            res.send({ status: 400, result: "Fail",error:error, message: "Product Name Already Exist" })
        else
            res.send({ status: 500, result: "Fail",error:error, message: "Internal Server Error",error:error })
    }
}
