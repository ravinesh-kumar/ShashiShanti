...
async function createRecord(req, res) {
    ...
    if (req.body.password && schema.validate(req.body.password)) {
            ...
            else 
            {
                ...
                try {
                    ...
                    mailOptions = {
                        ...
                        subject: "Account is Created : Team Shashi Shanti",
                        text: `
                                Hello ${data.name}
                ...
                catch (error)
                 {
                    ...
                    if (error.keyValue && error.keyValue.username)
                        ...
                        res.send({ status: 400, result: "Fail",error:error, message: "User Name Already Taken" })
                    else if (error.keyValue && error.keyValue.email)
                        res.send({ status: 400, result: "Fail",error:error, message: "Email Address Already Taken" })
                    else if (error.errors.name)
                        res.send({ status: 400, result: "Fail",error:error, message: error.errors.name.message })
                    else if (error.errors.email)
                        res.send({ status: 400, result: "Fail",error:error, message: error.errors.email.message })
                    else if (error.errors.phone)
                        res.send({ status: 400, result: "Fail",error:error, message: error.errors.phone.message })
                    else
                        res.send({ status: 500, result: "Fail",error:error, message: "Internal Server Error" })
                }
            }
        ...
        console.log("Password Validation Result: ", req.body.password,schema.validate(req.body.password));

        res.send({
            status: 400, result: "Fail",error:error, message: "Password Must Contains Following Items  1. Minimum length 2. Maximum length 3. Must have 1 uppercase letters 4. Must have 1 lowercase letters 5. Must have at least 1 digits 6. Should not have spaces"
        })}
}
...
async function updateRecord(req, res) {
    try {
        ...
        if (error.keyValue)
            res.send({ status: 400, result: "Fail",error:error, message: "User Name Already Exist" })
        else
            res.send({ status: 500, result: "Fail",error:error, message: "Internal Server Error" })
    }
}
    ...
    try {
        ...
        if (data) {
            if (await bcrypt.compare(req.body.password, data.password)) {
                ...
                // session expire in 15 days
                jwt.sign({ data }, key, { expiresIn }, (error, token) => {
                ...
                res.send({ status: 401, result: "Fail",error:error, message: "Invalid Username or Password" })
        }
        else
            res.send({ status: 401, result: "Fail",error:error, message: "Invalid Username or Password" })
    } catch (error) {
    ...
    try {
        ...
        if (data) {
            ...
            mailOptions = {
                from: process.env.MAIL_SENDER,
                to: data.email,
                subject: "OTP for Password Reset : Team Shashi Shanti",
                text: `
                        Hello ${data.name}
            ...
            res.send({ status: 200, result: "Done",error:error, message: "OTP Sent on Your Registered Email Address" })
        }
        else
            res.send({ status: 401, result: "Fail",error:error, message: "User Not Found" })
    } catch (error) {
        res.send({ status: 500, result: "Fail",error:error, message: "Internal Server Error" })
    }
}
    ...
    try {
        ...
        if (data) {
            if (data.otp == req.body.otp)
                ...
                res.send({ status: 401, result: "Fail",error:error, message: "Invalid OTP" })
        }
        else
            res.status(401).send({ status: 401, result: "Fail",error:error, message: "Anauthorized Activity" })
    } catch (error) {
        res.send({ status: 500, result: "Fail",error:error, message: "Internal Server Error" })
    }
}
        ...
        if (data) {
            bcrypt.hash(req.body.password, 12, async (error, hash) => {
                if (error)
                    ...
                    data.password = hash
                    await data.save()
                    res.send({ status: 200, result: "Done",error:error, message: "Password Has Been Reset" })
                }
            })
        }
        else
